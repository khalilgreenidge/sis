 <?php
    ...
    $savecontent = $_POST['assignment2'];
    $course = $_POST["course"];

    $loadcontent = $course."/info/assignment2.txt";
		...
		if($ext == "doc" || $ext == "docx" || $ext == "xls" || $ext == "ppt"){
			...
			for($i = 0; $i < count($courseArray); $i++){ 
				...
				if($theCourse != $trimCourse){
					...
					if( empty($xml->course[$i]->file[1]) ){
						$xml->course[$i]->addChild("file", $target_file)->addAttribute("id", "assignment2");
					}
					else{
						$xml->course[$i]->file[1] = $target_file;
					}	
